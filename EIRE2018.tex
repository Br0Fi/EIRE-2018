...
\fi
	...
	Überdies sind in \cref{MultiTraegerFrontpanel} noch fünf weitere Anzeigeelemente in Diagramm-Form dargestellt: Die beiden Oberen zeigen das über den Analog-Digital-Wandler in Kombination mit der Computer-Soundkarte gemessene amplitudenmodulierte Signal ohne Demodulation, zum einen im Zeitbild und zum andern im Frequenzbild. %TODO musst du mMn nich nochmal erklären, wenn es in der Caption steht. - Jannik: Stimme ich teilweise zu, aber ich lass es jetzt einfach stehen.
	...
	Bei der dazugehörigen Benutzeroberfläche, welche in \cref{FMAusgabekleinesk}, \cref{FMAusgabegrossesk} sowie \cref{PMAusgabe} dargestellt ist, befinden sich alle Bedien- bzw. Eingabeelemente auf der linken und alle Anzeige- bzw. Ausgabeelemente auf der rechten Seite.
	...
	Genauso wie in \cref{fig_tag3_am_soundkarte_block} aus \cref{AMSignalErzeugung} sieht man auf der linken Seite des Programmcodes zunächst die Erzeugung des Signals $s(t)$ gemäß \cref{Ursprungssignal}. 
	...
	Durch das Hinzufügen einer Case-Konstruktion kommt die Auswahlmöglichkeit zwischen Frequenz- und Phasenmodulation zustande. %TODO Vermerk - Jannik: "Vermerk" für was?
	...
	\begin{figure}[H]
		...
		\caption{Die Abbildung zeigt das Frontpanel bzw. die Benutzeroberfläche des LabVIEW-Programms zur Umsetzung der Frequenzmodulation. Der hierbei verwendete $k$-Wert ist sehr klein.
	...
	\noindent Die zwei dadurch erscheinenden, zunächst freien sowie ausfüllbaren Case-Felder sollen somit jeweils der Frequenzmodulation und der Phasenmodulation zur Verfügung stehen. %TODO natürlich sind sie zunächst frei. Wenn du normalen Code erklären würdest, wü+rdest du auch nicht sagen "Man drückt Enter. Die entstehende Zeile ist zunächst frei." - Jannik: Ich finde, dass es nötig ist einmal zu sagen, dass die anfangs frei sind, weil das weiß der Leser, dem das Programm ja unbekannt ist, nicht.
	...
	Innerhalb des für die Frequenzmodulation reservierten, freien, ausfüllbaren Case-Feldes (\glqq FM\grqq ) wird die in \cref{FMFormel} auftretende Integration von $s(t)$ ermöglicht. %TODO s.O. mit dem "frei, ausfüllbar" - Jannik: Ich hab's nur nochmal erwähnt, damit der Leser weiß, dass die oben angesprochenen Case-Felder gemeint sind.
	...
	%TODO Blödsinn, dass auf das offensichtliche Englische zu übersetzen. Du schreibst doch auch nicht Satz von Rolle (Rolle's theorem) - Jannik: Das war nicht das Ziel. Ich wollte auf die entsprechende Stelle im Programmcode hinweisen, hätte ich eindeutiger machen sollen, sry.
	
	\begin{figure}[H] %TODO Das ist doch bs, das abzubilden.schreib halt in das andere, dass der bei PM nur das Signal durchleitet. - Jannik: Hab ich unten auch geschrieben. Hab das Bild extra klein gemacht, ich finde das stört im Text jetzt auch nicht so wahnsinnig und zur Vollständigkeit gehört das schon irgendwie darein.
		\centering
	...
	\noindent Die gesamte Berechnung (FM-/PM-Modulation sowie Signalerzeugung) und der Großteil der Signalausgabe finden in einer While-Schleife statt, was auch aus \cref{FMProgrammcode} hervorgeht.
	...
	\noindent Daher besteht lediglich die Notwendigkeit, das für die Phasenmodulation reservierte, freie, ausfüllbare Case-Feld passend zu besetzen. %TODO s.O. mit frei ausfüllbar - Jannik: Stehen lassen, pls.
