...
\begin{document}
	...
	\begin{titlepage}
		\centering
		{\scshape\LARGE Protokoll zu \par} %TODO zur oder zu?
		\vspace{1cm}
...
%TODO am Ende: Begriffe vereinheitlichen und Größe von Grafiken anpassen
	...
	Demnach befindet sich $t$ auf der $x$-Achse und $u$ auf der $y$-Achse. %TODO wieso "demnach"? Das folgt doch nicht aus dem davor. Schreib halt "Dabei"
...
\fi
	...
	Das allgemeine Ziel dieses Abschnitts ist die Demodulation eines über den Analog-Digital-Wandler in Verbindung mit der Computer-Soundkarte gemessenen amplitudenmodulierten Signals unter Zuhilfenahme der Multiplikation mit der Trägerfrequenz. %gemessen wird nicht in Verbindung mit der Computer-Soundkarte, sonder es wird von ihr ausgegeben
	In der mathematischen Theorie führt das Multiplizieren der amplitudenmodulierten Signal-Funktion mit der Schwingungsfunktion, welche lediglich der Trägerfrequenz des AM-Signals unterliegt, dazu, dass sich mit dem passenden Additionstheorem als Ergebnis eine additive Aneinanderreihung von Schwingungstermen unterschiedlichster Frequenzen bildet. %TODO Schiwingungsfunktion? Warum nicht Trägerfunktion? Schwingen tut auch das Signal
	%Additive Aneinanderreihung? warum nicht überlagerung verschiedener Frequenzen? nicht dramatisch, klingt bloß nach mehr, als es ist.
	Letztere lassen sich unter Verwendung einer Fourier-Transformation im Frequenzspektrum dieses neu gewonnenen Schwingungsterms betrachten. %TODO Was ist "dieser neu gewonnene Schwingungsterm? Davor war von einer Aneinanderreihung von mehreren die Rede. Es wird nicht klar, welchen du davon meinst. ICh schätze, du meinst die des Signals, aber dann sag das halt
	Neben den diversen Peaks an Stellen mit vergleichsweise hohen Frequenzen, befinden sich darin ebenfalls Peaks bei relativ niedrigen Frequenzwerten. %TODO Link auf Diagramm?
		
	\begin{figure}[H] %TODO Warum nicht erst wo es relevant wird?
		\centering
		\includegraphics[width=1.0\textwidth]{EIRE2018Dateien/Tag4/traegerMultOszi/Oszilloskop__modifiziertd}
		\caption{Die Abbildung zeigt das LabVIEW-Blockdiagramm bzw. den Programmcode zur Demodulation eines über den Analog-Digital-Wandler in Kombination mit der Computer-Soundkarte gemessenen amplitudenmodulierten Signals mittels Trägerfrequenzmultiplikation.} %s.O.
		\label{MultiTraegerProgrammcode}
	\end{figure}

	\noindent Demnach handelt es sich um ein breites Frequenzspektrum, in dem man sogar die Frequenz(en) des modulierten, ursprünglichen Signals deutlich erkennen kann. %TODO sogar? Das ist doch gerade das Ziel. Das Wort würde ich streichen.
	Von einem theoretischen Standpunkt aus ist die komplette Demodulation des AM-Signals damit abgeschlossen. %naja, theoretisch ist zum Glück Definitionssache...
	...
	Dies entspricht dem Vorgang bei einem herkömmlichen Radio den Sender einzustellen. %hab ich eingefügt.
	...
	Überdies sind in \cref{MultiTraegerFrontpanel} noch fünf weitere Anzeigeelemente in Diagramm-Form dargestellt: Die beiden Oberen zeigen das über den Analog-Digital-Wandler in Kombination mit der Computer-Soundkarte gemessene amplitudenmodulierte Signal ohne Demodulation, zum einen im Zeitbild und zum andern im Frequenzbild. %musst du mMn nich nochmal erklären, wenn es in der Caption steht.
	...
	Hierbei liegt nämlich eine Versechsfachung des Modulationsgrads $m$ vor. %TODO Man muss dem Leser nicht das Anwenden von Grundschulmathematik absprechen. Lass entweder den Satz oder das nämlich weg.
	...
	Bei der dazugehörigen Benutzeroberfläche, welche unter anderem in \cref{FMAusgabekleinesk}, \cref{FMAusgabegrossesk} sowie \cref{PMAusgabe} dargestellt ist, befinden sich alle Bedien- bzw. Eingabeelemente auf der linken und alle Anzeige- bzw. Ausgabeelemente auf der rechten Seite. %TODO "unter anderem" ist gelogen. Es ist nur da.
	...
	\end{figure}
	%TODO "(Ursprungs-)Signalentstehung" steht hier sehr oft und klingt sehr komisch. Entweder mit Ursprung oder ohne. Und der macht häufig die schließende Klammer an den Anfang der Zeile. Das ist mit Sicherheit nicht korrekt.
	...
	Durch das Hinzufügen einer Case-Konstruktion kommt die Auswahlmöglichkeit zwischen Frequenz- und Phasenmodulation zustande. %TODO Vermerk
	...
	\begin{figure}[H]
		...
		\caption{Die Abbildung zeigt das Frontpanel bzw. die Benutzeroberfläche des LabVIEW-Programms zur Umsetzung der Frequenzmodulation. Der hierbei verwendete $k$-Wert ist relativ klein. %relativ zu was? Hast du eine Ahnung, was in der Praxis verwendet wird? Nein hast du nicht. Du findest es nur klein, weil da viele nullen sind... Wären es Nano-k wäre es groß.
	...
	\noindent Die zwei dadurch erscheinenden, zunächst freien sowie ausfüllbaren Case-Felder sollen somit jeweils der Frequenzmodulation und der Phasenmodulation zur Verfügung stehen. %TODO natürlich sind sie zunächst frei. Wenn du normalen Code erklären würdest, wü+rdest du auch nicht sagen "Man drückt Enter. Die entstehende Zeile ist zunächst frei."
	Der Wechsel zwischen diesen beiden erfolgt durch ein mit dem Namen \glqq Modulationsart\grqq\ gekennzeichnetes Bedienelement auf der Benutzeroberfläche samt dazugehörigem Controler an der entsprechenden Stelle im Programmcode. %TODO war das nicht nur Control, oder so? Lass dich gar nicht erst auf solche Begriffe ein...
	...
	Innerhalb des für die Frequenzmodulation reservierten, freien, ausfüllbaren Case-Feldes (\glqq FM\grqq) wird die in \cref{FMFormel} auftretende Integration von $s(t)$ ermöglicht. %TODO s.O. mit dem "frei, ausfüllbar
	Dabei wird auf die Simpsonregel (\glqq Simpson's Rule\grqq ) zurückgegriffen, da diese im Vergleich zu anderen bekannten Verfahren der numerischen Integration am effizientesten ist. %TODO wowowow, mit dem Statement wäre ich vorsichtig. Trapeze sind glaube ich schneller, aber ungenauer und irgendwelche anderen sind maybe langsamer aber genauer.
	%Blödsinn, dass auf das offensichtliche Englische zu übersetzen. Du schreibst doch auch nicht Satz von Rolle (Rolle's theorem)
	
	\begin{figure}[H] %Das ist doch bs, das abzubilden.schreib halt in das andere, dass der bei PM nur das Signal durchleitet.
		\centering
	...
	\noindent Die gesamte Berechnung (FM-/PM-Modulation sowie (Ursprungs-)Signalerzeugung) und der Großteil der Signalausgabe finden in einer While-Schleife statt, was auch aus \cref{FMProgrammcode} hervorgeht. %Klammern in Klammern würd ich dir als Deutschlehrer ankreiden. Der Physiker mag's vielleicht, aber siehe oben für (Ursprungs-)Signalerzeugung
	...
	Der Vergleich der beiden Frequenzbilder legt die Vermutung nahe, dass die Vergrößerung des $k$-Wertes zu schärfer definierten Peaks im Frequenzspektrum führt.%geht auch offensichtlich aus der Gleichung hervor.
	...
	\noindent Daher besteht lediglich die Notwendigkeit, das für die Phasenmodulation reservierte, freie, ausfüllbare Case-Feld passend zu besetzen. %s.O. mit frei ausfüllbar
	...
	\emph{Hinweis:} Bei diesem LabVIEW-Programm zur Erzeugung von phasen- und frequenzmodulierten Signalen ist zu beachten, dass zwischen den $k$-Werten der Frequenzmodulation und den $k$-Werten der Phasenmodulation stets eine Diskrepanz um einen Faktor der Größenordnung $10^4$ vorliegt. %TODO ...weil das zu den verlässlichsten Ergebnissen führt. Wäre zu erwähnen, damit das nicht abiträr wirkt.
	
	
	%TODO Leonut hier ansetzen
	...
	Der Wechsel zwischen diesen beiden Optionen findet dabei mit Hilfe eines sich auf der Benutzeroberfläche befindenden Kippschalters statt, welcher die der beiden reservierten Case-Feld-Reservierungen entsprechenden Markierungen \glqq FM\grqq\ und \glqq PM\grqq\ besitzt.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.0\textwidth]{EIRE2018Dateien/Tag4/OsziFMPM-Demod/FM/OsziPlusFMPMd}
		\caption{Die Abbildung veranschaulicht das LabVIEW-Blockdiagramm bzw. den Programmcode zur Demodulation eines über den Analog-Digital-Wandler und die Computer-Soundkarte gemessenen, phasen- bzw. frequenzmodulierten Signals.}
...
>>>>>>> 3614de43e05eab2706921412f349b6bb2c742fc2
	\end{figure}
...
<<<<<<< HEAD
		\caption{In dieser Abbildung ist das Ergebnis der Demodulation und Tiefpassfilterung des PM-Signals zu sehen. Dieses Frequenzbild mit Hanning-Fenster ist das Äquivalent zu dem aus \cref{FMPMDemodFrontpanel} bekannten, sich rechts unten befindenden Frontpanel-Diagramm. Die Daten für das hier dargestellte Frequenzspektrum entspringen der durch die Speicherfunktion des LabVIEW-Programms erhaltenen txt-Datei und wurden mit dem Datenanalyse-Programm \glqq SciDAVis\grqq\ bearbeitet.}
=======
